...
//		};
	...
	public void move(Move m, boolean isOfficial) {
		...
		if(isOfficial) {
			...
			//if a piece has been captured or a pawn has been moved, clear history
	...
	public ArrayList<Move> findBishopMoves(Position pos){
		...
		PieceData p = getPiece(pos);
		if(p == null) {
			System.out.println(pos + " is not a valid position, you done FCUKED UP");
	 ...
	 */
	public ArrayList<Move> findLegalMoves(int side,PieceData[][] board){
		ArrayList<Move> moveList = new ArrayList<>();
		PieceData p;
		Position pos;
		Position kingPos = new Position(-1,-1);
		
		ArrayList<PieceData> listOfPiecesTestDeleteMe = new ArrayList<PieceData>();
		ArrayList<PieceData> listOfPiecesTest2DeleteMe = new ArrayList<PieceData>();
		...
		for(int y = 0; y < board.length; y++) {
			for(int x = 0; x < board[y].length; x++) {
				...
				if(p.getPiece().getSide() == side) {
					...
					listOfPiecesTestDeleteMe.add(p);
					...
					listOfPiecesTest2DeleteMe.add(p);
				}
			}
		}
		
		if(kingPos.equals(new Position(-1,-1)) ) {
			//System.out.println("King has not been found");
			System.out.println("My side: " + listOfPiecesTestDeleteMe.toString());
			System.out.println("Other side: " + listOfPiecesTest2DeleteMe.toString());
			throw new NullPointerException("King has not been found for side " + side);
		}
	...
	public GameResult getStatus() {
		return getStatus(boardHistory,moveHistory,board);
	}
	public GameResult getStatus(PieceData[][] board) {
		return getStatus(boardHistory,moveHistory,board);
	}
	 ...
	 */
	public GameResult getStatus(ArrayList<PieceData[][]> boardHistory, ArrayDeque<Move> moveHistory, PieceData[][] board) {
		...
		if(boardHistory.size() >= 50) {
			...
			ArrayList<Move> moveList = findLegalMoves(side,board);
	...
	public boolean isThreefoldRepetitionDraw(ArrayList<PieceData[][]> boardHistory) {
		...
		for(int i = 0; i < boardHistory.size(); i++) {
			numSamePos = 1;
			for(int j = i+1; j < boardHistory.size(); j+=1) {
				if(boardEquals(boardHistory.get(i), boardHistory.get(j))) {
					numSamePos++;
				}
			}
			if(numSamePos >= 3) {
	 ...
	 * @return gets the piece of the working board, aka the one not used in minmax
	 */
	public PieceData getWorkingPiece(Position p) {
		if(p.y >= 0 && p.y < workingBoard.length && p.x >= 0 && p.x < workingBoard[0].length) {
			return workingBoard[p.y][p.x];
		}
		return null;
	}
	...
	public String toString(PieceData[][] board) {
